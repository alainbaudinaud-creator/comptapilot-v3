...
def analyse_ia_document(document_id):
    ...
    for r in regles:
        mot_cle, compte, journal, type_regle, commentaire = r
...
def grand_livre_v3():
    ...
    c.execute("""
        SELECT compte, journal, date_ecriture, libelle, debit, credit
        FROM ecritures_auto
...
def bilan_v3():
    ...
    c.execute("""
        SELECT compte, COALESCE(SUM(debit-credit),0)
        FROM ecritures_auto
        GROUP BY compte
    """)
    ...
    for compte, solde in rows:
        compte = str(compte or "")
...
def detection_fraude_v3():
    ...
    c.execute("""
        SELECT id, journal, date_ecriture, libelle, compte, debit, credit
        FROM ecritures_auto
...
def controle_fec_dgfip_v3():
    ...
    c.execute("""
        SELECT COUNT(*)
        FROM ecritures_auto
        WHERE compte IS NULL OR compte = ''
    """)
...
def assistant_conversationnel_comptable_v3():
    ...
    if request.method == 'POST':
        ...
        if "tva" in question:
            c.execute("SELECT COALESCE(SUM(debit),0) FROM ecritures_auto WHERE compte LIKE '44566%'")
            tva_deductible = c.fetchone()[0] or 0
            c.execute("SELECT COALESCE(SUM(credit),0) FROM ecritures_auto WHERE compte LIKE '44571%'")
            tva_collectee = c.fetchone()[0] or 0
...
def lettrage_automatique_v4():
    ...
    c.execute("""
        SELECT compte, libelle, ABS(debit-credit)
        FROM ecritures_auto
        WHERE compte LIKE '401%' OR compte LIKE '411%'
    """)

    lignes = c.fetchall()
    date_now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    for compte, libelle, montant in lignes:
        c.execute("""
            INSERT INTO lettrages_auto_v4 (compte, libelle, montant, statut, date_lettrage)
            VALUES (?, ?, ?, ?, ?)
        """, (compte, libelle, montant, "LETTRAGE_SUGGERE", date_now))
...
def liste_lettrages_v4():
    ...
    c.execute("""
        SELECT id, compte, libelle, montant, statut, date_lettrage
...
def ajouter_immobilisation_v4():
    ...
    compte = request.form.get('compte') or "218"
...
def analytique_v4():
    ...
    c.execute("""
        SELECT id, axe, compte, montant, commentaire
        FROM analytique_v4
        ORDER BY id DESC
    """)
...
def ajouter_analytique_v4():
    ...
    c.execute("""
        INSERT INTO analytique_v4 (axe, compte, montant, commentaire)
        VALUES (?, ?, ?, ?)
    """, (axe, compte, montant, commentaire))
...
def initialiser_regles_comptables():
    ...
    if nb == 0:
        ...
        for mot, compte, libelle in regles:
...
def fec_export():
    ...
    for row in rows:
        ...
        if not compte_num:
            anomalies.append(f"Ecriture {ecriture_id} sans compte comptable")
...
def cloture_comptable():
    ...
    if nb_sans_compte > 0:
        alertes.append(f"{nb_sans_compte} écriture(s) sans compte comptable.")
    ...
    for r in rows:
        ...
        if compte == "":
            anomalies.append({
                "niveau": "Critique",
                "type": "Compte absent",
                "message": "Aucun compte comptable",
                "piece": piece,
                "compte": "-",
                "montant": mouvement
            })
...
def reanalyser_pieces():
    ...
    for piece in pieces:
        ...
        for i, ligne in enumerate(lignes_ocr):
            ...
            if ligne.lower() == "emetteur" and i + 1 < len(lignes_ocr):
                fournisseur_detecte = lignes_ocr[i + 1]
                score_ia += 25

            if ligne.lower().startswith("facture") and not numero_facture_detecte:
                numero_facture_detecte = ligne
                score_ia += 20

            if "tva" in ligne.lower() and not tva_detectee:
                tva_detectee = ligne
                score_ia += 20
