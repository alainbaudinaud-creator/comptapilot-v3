...
def validation_ia(piece_id):
    ...
    if request.method == "POST":
        ...
        facture = request.form.get("facture", "").strip()
        date_piece = request.form.get("date_piece", "").strip()
        ttc = request.form.get("ttc", "").strip()
        tva = request.form.get("tva", "").strip()
        compte_charge = request.form.get("compte_charge", "606300").strip()
        ...
        return redirect(f"/ecritures/creer-ecriture-ia/{piece_id}")
...
def creer_ecriture_ia(piece_id):
    ...
    deja_liee = c.fetchone()

    if deja_liee and deja_liee[0]:

        conn.close()

        flash("Une écriture existe déjà pour cette pièce.", "warning")
    ...
    if doublon:
        ...
        flash("Doublon évité : une écriture existe déjà pour cette facture.", "warning")
