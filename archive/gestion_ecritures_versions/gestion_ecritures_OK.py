...
def reanalyser_pieces():
    ...
    for piece in pieces:
        ...
        if not date_detectee:
            match_date_mois_jour = re.search(
                r"\b(janv\.|janvier|févr\.|fevr\.|février|fevrier|mars|avr\.|avril|mai|juin|juil\.|juillet|août|aout|sept\.|septembre|oct\.|octobre|nov\.|novembre|déc\.|dec\.|décembre|decembre)\s+([0-9]{1,2}),\s*([0-9]{4})\b", 
                texte_ocr,
                re.IGNORECASE
            )

        if match_date_mois_jour:

            jour = str(
                int(match_date_mois_jour.group(2))
            ).zfill(2)

            mois_txt = (
                match_date_mois_jour
                .group(1)
                .lower()
                .strip()
            )

            annee = match_date_mois_jour.group(3)
            ...
            mois_num = mois_map.get(mois_txt, "01")
            date_detectee = f"{jour}/{mois_num}/{annee}"
