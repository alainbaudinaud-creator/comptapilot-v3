...
from repositories.ecritures.ecritures_repository import fetch_ecritures, societe_est_cloturee_repository, ecriture_verrouillee_repository, fetch_ecritures_export_dashboard
from repositories.ecritures.stats_repository import fetch_global_stats, fetch_dashboard_financier_par_type
from services_v3.dashboard.dashboard_service import get_dashboard_financier
from repositories.sql_stats_repository import fetch_count_safe
from services_v3.supervision.supervision_service import get_supervision_page_data
from repositories.ecritures.stats_repository import fetch_global_stats, fetch_dashboard_financier_par_type
from services_v3.dashboard.dashboard_service import get_dashboard_financier
from repositories.sql_stats_repository import fetch_count_safe
from services_v3.supervision.supervision_service import get_supervision_page_data
...
def ecriture_verrouillee(c, ecriture_id):
...
@permission_required("ACCESS_ECRITURES")
@ecritures_routes.route('/controle-?quilibre-global-v5')
@login_required
...
def supprimer_ecriture(ecriture_id):
    ...
    if ecriture_verrouillee(c, ecriture_id):
        conn.close()
        flash("Suppression impossible : ?criture verrouill?e.", "danger")
...
@ecritures_routes.route('/controle-verrouillage')
@login_required
@permission_required("ACCESS_ECRITURES")
def controle_verrouillage():
    ...
    c.execute("""
        SELECT
            COUNT(*),
            SUM(CASE WHEN verrouillee=1 THEN 1 ELSE 0 END),
            SUM(CASE WHEN verrouillee=0 OR verrouillee IS NULL THEN 1 ELSE 0 END)
        FROM ecritures
    """)

    total, verrouillees, non_verrouillees = c.fetchone()

    conn.close()

    return f"""
    <h1>Contr?le verrouillage comptable</h1>
    <p>Total ?critures : {total}</p>
    <p>?critures verrouill?es : {verrouillees}</p>
    <p>?critures non verrouill?es : {non_verrouillees}</p>
    <p><a href='/ecritures/'>Retour aux ?critures</a></p>
    """



@ecritures_routes.route('/controle-clotures')
@login_required
@permission_required("ACCESS_ECRITURES")
def controle_clotures():
...
@ecritures_routes.route('/dossiers-comptables')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossiers_comptables():

    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()

    c.execute("""
        SELECT id, nom, siret, exercice_courant, actif
        FROM dossiers_comptables
        ORDER BY id
    """)
    ...
    for d in dossiers:
        ...
        <tr>
            ...
            <td>{d[2] or ''}</td>
            <td>{d[3]}</td>
            <td>{'Oui' if d[4] else 'Non'}</td>
            <td><a href="/ecritures/changer-dossier/{d[0]}">Choisir</a></td>
        </tr>
        """

    html += """
    </table>
    <p><a href="/ecritures/">Retour</a></p>
    """
...
def get_dossier_courant():

    dossier_id = session.get("dossier_id")

    if not dossier_id:
        dossier_id = 1
        session["dossier_id"] = 1

    return dossier_id


@ecritures_routes.route('/changer-dossier/<int:dossier_id>')
@login_required
@permission_required("ACCESS_ECRITURES")
def changer_dossier(dossier_id):

    session["dossier_id"] = dossier_id

    flash("Dossier comptable chang?.", "success")

    return redirect("/ecritures/dossiers-comptables")



@ecritures_routes.route('/dossier/balance')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_balance():

    dossier_id = get_dossier_courant()
    ...
    c.execute("""
        SELECT
            pc.numero,
            pc.libelle,
            ROUND(SUM(COALESCE(e.debit,0)),2),
            ROUND(SUM(COALESCE(e.credit,0)),2)
        FROM ecritures e
        LEFT JOIN plan_comptable pc ON e.compte_id=pc.id
        WHERE COALESCE(e.dossier_id,1)=?
        GROUP BY pc.numero, pc.libelle
        ORDER BY pc.numero
    """, (dossier_id,))
    ...
    total_debit = round(sum(float(l[2] or 0) for l in lignes), 2)
    total_credit = round(sum(float(l[3] or 0) for l in lignes), 2)
    ecart = round(total_debit - total_credit, 2)

    conn.close()

    html = f"<h1>Balance dossier {dossier_id}</h1>"
    html += f"<p>Total d?bit : {total_debit}</p><p>Total cr?dit : {total_credit}</p><p>?cart : {ecart}</p>"
    html += "<table border='1' cellpadding='8'><tr><th>Compte</th><th>Libell?</th><th>D?bit</th><th>Cr?dit</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0] or ''}</td><td>{l[1] or ''}</td><td>{l[2]}</td><td>{l[3]}</td></tr>"

    html += "</table><p><a href='/ecritures/dossiers-comptables'>Retour dossiers</a></p>"

    return html


@ecritures_routes.route('/dossier/grand-livre')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_grand_livre():
    ...
    html = f"<h1>Grand livre dossier {dossier_id}</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>Date</th><th>Pi?ce</th><th>Compte</th><th>Nom compte</th><th>Libell?</th><th>D?bit</th><th>Cr?dit</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2] or ''}</td><td>{l[3] or ''}</td><td>{l[4]}</td><td>{l[5]}</td><td>{l[6]}</td></tr>"
...
@ecritures_routes.route('/dossier/journaux')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_journaux():
    ...
    html = f"<h1>Journaux dossier {dossier_id}</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>Journal</th><th>Date</th><th>Pi?ce</th><th>Libell?</th><th>Compte</th><th>D?bit</th><th>Cr?dit</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td><td>{l[3]}</td><td>{l[4] or ''}</td><td>{l[5]}</td><td>{l[6]}</td></tr>"
...
@ecritures_routes.route('/dossier/pieces-jointes')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_pieces_jointes():
    ...
    c.execute("""
        SELECT id, nom_fichier, date_upload, fournisseur_detecte,
               numero_facture_detecte, date_detectee,
               montant_ttc_detecte, tva_detectee, statut_ia
        FROM pieces_jointes
        WHERE COALESCE(dossier_id,1)=?
        ORDER BY id DESC
    """, (dossier_id,))
    ...
    html = f"<h1>Pi?ces justificatives dossier {dossier_id}</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>ID</th><th>Fichier</th><th>Date upload</th><th>Fournisseur</th><th>Facture</th><th>Date</th><th>TTC</th><th>TVA</th><th>Statut</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td><td>{l[3] or ''}</td><td>{l[4] or ''}</td><td>{l[5] or ''}</td><td>{l[6] or ''}</td><td>{l[7] or ''}</td><td>{l[8] or ''}</td></tr>"
...
@ecritures_routes.route('/dossier/tva')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_tva():
    ...
    html = f"<h1>TVA dossier {dossier_id}</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>Compte</th><th>Libell?</th><th>D?bit</th><th>Cr?dit</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td><td>{l[3]}</td></tr>"
...
@ecritures_routes.route('/dossier/clotures')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_clotures():

    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()

    c.execute("""
        SELECT id, date_cloture, exercice, total_debit, total_credit, ecart, statut
        FROM clotures_comptables
        ORDER BY id DESC
    """)
    ...
    html = "<h1>Cl?tures comptables</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>ID</th><th>Date</th><th>Exercice</th><th>D?bit</th><th>Cr?dit</th><th>?cart</th><th>Statut</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td><td>{l[3]}</td><td>{l[4]}</td><td>{l[5]}</td><td>{l[6]}</td></tr>"
...
@ecritures_routes.route('/dossier/dashboard')
@login_required
@permission_required("ACCESS_ECRITURES")
def dossier_dashboard():
    ...
    nb_pieces = c.fetchone()[0]

    conn.close()

    ecart = round((debit or 0) - (credit or 0), 2)

    return f"""
    <h1>Dashboard dossier {dossier_id}</h1>
    <p>?critures : {total}</p>
    <p>Pi?ces justificatives : {nb_pieces}</p>
    <p>Total d?bit : {debit}</p>
    <p>Total cr?dit : {credit}</p>
    <p>?cart : {ecart}</p>
    <p><a href='/ecritures/dossier/journaux'>Journaux</a></p>
    <p><a href='/ecritures/dossier/grand-livre'>Grand livre</a></p>
    <p><a href='/ecritures/dossier/balance'>Balance</a></p>
    <p><a href='/ecritures/dossier/tva'>TVA</a></p>
    <p><a href='/ecritures/dossier/pieces-jointes'>Pi?ces justificatives</a></p>
    """



@ecritures_routes.route('/production/health')
@login_required
@permission_required("ACCESS_ECRITURES")
def production_health():
    ...
    c.execute("SELECT COUNT(*) FROM pieces_jointes")
    nb_pieces = c.fetchone()[0]
    ...
    c.execute("SELECT COUNT(*) FROM dossiers_comptables")
    nb_dossiers = c.fetchone()[0]

    conn.close()

    statut = "OK" if ecart == 0 else "ANOMALIE"

    return f"""
    <h1>Sant? production ComptaPilot</h1>
    <p>Statut comptable : <strong>{statut}</strong></p>
    <p>Dossiers : {nb_dossiers}</p>
    <p>?critures : {nb_ecritures}</p>
    <p>Pi?ces justificatives : {nb_pieces}</p>
    <p>Total d?bit : {debit}</p>
    <p>Total cr?dit : {credit}</p>
    <p>?cart : {ecart}</p>
    <p><a href="/ecritures/dossier/dashboard">Dashboard dossier</a></p>
    """


@ecritures_routes.route('/production/audit')
@login_required
@permission_required("ACCESS_ECRITURES")
def production_audit():
    ...
    c.execute("""
        SELECT id, date_ecriture, piece, libelle, compte_id, debit, credit
        FROM ecritures
        ORDER BY id
    """)
    ...
    for r in rows:
        id_, date_ecriture, piece, libelle, compte_id, debit, credit = r

        if not date_ecriture:
            erreurs.append((id_, "Date manquante"))
        if not piece:
            erreurs.append((id_, "Pi?ce manquante"))
        if not libelle:
            erreurs.append((id_, "Libell? manquant"))
        if not compte_id:
            erreurs.append((id_, "Compte manquant"))

        debit = float(debit or 0)
        credit = float(credit or 0)

        if debit == 0 and credit == 0:
            erreurs.append((id_, "D?bit et cr?dit ? z?ro"))
        if debit > 0 and credit > 0:
            erreurs.append((id_, "D?bit et cr?dit remplis ensemble"))
    ...
    html = "<h1>Audit comptable production</h1>"
    html += f"<p>Total d?bit : {debit}</p>"
    html += f"<p>Total cr?dit : {credit}</p>"
    html += f"<p>?cart : {ecart}</p>"
    html += f"<p>Nombre anomalies : {len(erreurs)}</p>"

    if erreurs:
        html += "<table border='1' cellpadding='8'><tr><th>ID</th><th>Anomalie</th></tr>"
        for e in erreurs:
            html += f"<tr><td>{e[0]}</td><td>{e[1]}</td></tr>"
        html += "</table>"
    else:
        html += "<p><strong>Aucune anomalie d?tect?e.</strong></p>"

    return html


@ecritures_routes.route('/production/sauvegarde')
@login_required
@permission_required("ACCESS_ECRITURES")
def production_sauvegarde():
    ...
    destination = f"backup_comptapilot_{horodatage}.sqlite"

    shutil.copyfile(source, destination)

    return f"""
    <h1>Sauvegarde cr??e</h1>
    <p>Fichier : {destination}</p>
    <p><a href="/ecritures/production/health">Retour sant? production</a></p>
    """


@ecritures_routes.route('/production/centre-controle')
@login_required
@permission_required("ACCESS_ECRITURES")
def production_centre_controle():
    ...
    <body>
        ...
        <h1>Centre de controle production</h1>
        <p class="subtitle">
            Pilotage technique, comptable et SaaS de ComptaPilot.
        </p>
        ...
        <div class="grid">
            ...
            <div class="card">
                <h2>Sante systeme</h2>
                <p>Controle general de la base, des dossiers, des pieces et de l'equilibre comptable.</p>
                <a class="btn" href="/ecritures/production/health">Ouvrir</a>
            </div>

            <div class="card">
                <h2>Audit comptable</h2>
                <p>Recherche des anomalies FEC : dates, comptes, libelles, debit et credit.</p>
                <a class="btn" href="/ecritures/production/audit">Ouvrir</a>
            </div>

            <div class="card">
                <h2>Sauvegarde</h2>
                <p>Cree une copie horodatee de la base comptable SQLite.</p>
                <a class="btn" href="/ecritures/production/sauvegarde">Sauvegarder</a>
            </div>

            <div class="card">
                <h2>Dossiers comptables</h2>
                <p>Gestion et selection du dossier courant pour l'isolation multi-societes.</p>
                <a class="btn" href="/ecritures/dossiers-comptables">Ouvrir</a>
            </div>

            <div class="card">
                <h2>Dashboard dossier</h2>
                <p>Vue synthetique du dossier actif : ecritures, pieces, debit, credit, ecart.</p>
                <a class="btn" href="/ecritures/dossier/dashboard">Ouvrir</a>
            </div>

            <div class="card">
                <h2>Export FEC</h2>
                <p>Generation du fichier FEC CSV exploitable pour controle et export cabinet.</p>
                <a class="btn" href="/ecritures/export-fec">Telecharger</a>
            </div>

            <div class="card">
                <h2>Verrouillage</h2>
                <p>Controle des ecritures verrouillees pour securiser les exports et clotures.</p>
                <a class="btn" href="/ecritures/controle-verrouillage">Ouvrir</a>
            </div>

            <div class="card">
                <h2>Clotures</h2>
                <p>Historique des clotures comptables et controles d'equilibre.</p>
                <a class="btn" href="/ecritures/controle-clotures">Ouvrir</a>
            </div>
...
@ecritures_routes.route('/saas/pilotage')
@login_required
@permission_required("ACCESS_ECRITURES")
def saas_pilotage():
    ...
    c.execute("SELECT COUNT(*) FROM clients_saas")
    ...
    <head>
        <meta charset="utf-8">
        <title>Pilotage SaaS</title>
        <style>
            body{{font-family:Arial;background:#f4f6f9;padding:40px;color:#1f2d3d;}}
            h1{{color:#173b66;}}
            .grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px;max-width:1000px;}}
            .card{{background:white;padding:22px;border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,.08);border-left:5px solid #173b66;}}
            .num{{font-size:32px;font-weight:bold;color:#173b66;}}
            a.btn{{display:inline-block;margin-top:25px;background:#173b66;color:white;padding:10px 14px;border-radius:6px;text-decoration:none;}}
        </style>
    </head>
    <body>
        <h1>Pilotage SaaS ComptaPilot</h1>
        <div class="grid">
            <div class="card"><div>Clients SaaS</div><div class="num">{nb_clients}</div></div>
            <div class="card"><div>Dossiers comptables</div><div class="num">{nb_dossiers}</div></div>
            <div class="card"><div>Ecritures</div><div class="num">{nb_ecritures}</div></div>
            <div class="card"><div>Pieces justificatives</div><div class="num">{nb_pieces}</div></div>
        </div>
        <a class="btn" href="/ecritures/production/centre-controle">Centre production</a>
    </body>
    </html>
    """


@ecritures_routes.route('/saas/clients')
@login_required
@permission_required("ACCESS_ECRITURES")
def saas_clients():
    ...
    c.execute("""
        SELECT id, nom, email, formule, statut, date_creation
        FROM clients_saas
        ORDER BY id DESC
    """)
    ...
    <h1>Clients SaaS</h1>
    <table border="1" cellpadding="8">
    <tr><th>ID</th><th>Nom</th><th>Email</th><th>Formule</th><th>Statut</th><th>Date creation</th></tr>
    """

    for c in clients:
        html += f"<tr><td>{c[0]}</td><td>{c[1]}</td><td>{c[2]}</td><td>{c[3]}</td><td>{c[4]}</td><td>{c[5]}</td></tr>"

    html += "</table><p><a href='/ecritures/saas/pilotage'>Retour pilotage SaaS</a></p>"

    return html



@ecritures_routes.route('/api/production/health')
@login_required
@permission_required("ACCESS_ECRITURES")
def api_production_health():
    ...
    return jsonify({
        "status": "OK" if ecart == 0 else "ANOMALIE",
...
@ecritures_routes.route('/api/saas/clients')
@login_required
@permission_required("ACCESS_ECRITURES")
def api_saas_clients():
...
@ecritures_routes.route('/api/dossier/dashboard')
@login_required
@permission_required("ACCESS_ECRITURES")
def api_dossier_dashboard():
    ...
    nb_ecritures, debit, credit = c.fetchone()
...
@ecritures_routes.route('/api/production/audit')
@login_required
@permission_required("ACCESS_ECRITURES")
def api_production_audit():
    ...
    for r in rows:
        ...
        if not date_ecriture:
            erreurs.append({"id": id_, "anomalie": "Date manquante"})
        if not piece:
            erreurs.append({"id": id_, "anomalie": "Piece manquante"})
        if not libelle:
            erreurs.append({"id": id_, "anomalie": "Libelle manquant"})
        if not compte_id:
            erreurs.append({"id": id_, "anomalie": "Compte manquant"})

        debit = float(debit or 0)
        credit = float(credit or 0)

        if debit == 0 and credit == 0:
            erreurs.append({"id": id_, "anomalie": "Debit et credit a zero"})
        if debit > 0 and credit > 0:
            erreurs.append({"id": id_, "anomalie": "Debit et credit remplis ensemble"})

    conn.close()

    return jsonify({
        "nombre_anomalies": len(erreurs),
        "anomalies": erreurs
    })



@ecritures_routes.route('/demo/commerciale')
@login_required
@permission_required("ACCESS_ECRITURES")
def demo_commerciale():
    ...
    statut = "SYSTEME STABLE" if ecart == 0 else "ANOMALIE"
    ...
    <body>
        ...
        <div class="topbar">
            <h1>ComptaPilot</h1>
            <p>ERP comptable SaaS assiste par IA</p>
        </div>
        ...
        <div class="container">
            ...
            <div class="grid">
                ...
                <div class="card">
                    ...
                    <div class="big ok">{statut}</div>
                </div>

                <div class="card">
                    <h2>Dossiers</h2>
                    <div class="big">{nb_dossiers}</div>
                </div>

                <div class="card">
                    <h2>Ecritures</h2>
                    <div class="big">{nb_ecritures}</div>
                </div>

                <div class="card">
                    <h2>Pieces OCR</h2>
                    <div class="big">{nb_pieces}</div>
                </div>
            ...
            <div class="actions">
                ...
                <div class="action">
                    <h3>Workflow OCR IA</h3>
                    <p>Upload facture ? analyse IA ? validation ? ecriture comptable.</p>
                    <a class="btn" href="/ecritures/pieces-jointes">Ouvrir</a>
                </div>

                <div class="action">
                    <h3>Grand livre</h3>
                    <p>Consultation comptable complete par dossier.</p>
                    <a class="btn" href="/ecritures/dossier/grand-livre">Ouvrir</a>
                </div>

                <div class="action">
                    <h3>Balance comptable</h3>
                    <p>Controle debit / credit et equilibre global.</p>
                    <a class="btn" href="/ecritures/dossier/balance">Ouvrir</a>
                </div>

                <div class="action">
                    <h3>Export FEC</h3>
                    <p>Generation FEC CSV pour cabinet et controle fiscal.</p>
                    <a class="btn" href="/ecritures/export-fec">Exporter</a>
                </div>

                <div class="action">
                    <h3>Centre production</h3>
                    <p>Audit, sauvegarde, monitoring et supervision SaaS.</p>
                    <a class="btn" href="/ecritures/production/centre-controle">Ouvrir</a>
                </div>

                <div class="action">
                    <h3>Pilotage SaaS</h3>
                    <p>Clients, dossiers, supervision ERP et statistiques.</p>
                    <a class="btn" href="/ecritures/saas/pilotage">Ouvrir</a>
                </div>
...
@ecritures_routes.route('/pro/dashboard')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_dashboard():
    ...
    <head>
        <meta charset="utf-8">
        <title>ComptaPilot Pro</title>
        <style>
            body{font-family:Arial;background:#f4f6f9;margin:0;color:#1f2d3d;}
            .top{background:#173b66;color:white;padding:28px 40px;}
            .top h1{margin:0;font-size:34px;}
            .wrap{padding:40px;}
            .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;}
            .card{background:white;border-radius:12px;padding:24px;box-shadow:0 2px 10px rgba(0,0,0,.08);border-left:5px solid #173b66;}
            .card h2{margin-top:0;color:#173b66;}
            .btn{display:inline-block;background:#173b66;color:white;padding:10px 14px;border-radius:6px;text-decoration:none;font-weight:bold;}
        </style>
    </head>
    <body>
        <div class="top">
            <h1>ComptaPilot Pro</h1>
            <p>Console de professionnalisation SaaS</p>
        </div>
        <div class="wrap">
            <div class="grid">
                <div class="card"><h2>Utilisateurs et roles</h2><p>Gestion des profils plateforme.</p><a class="btn" href="/ecritures/pro/utilisateurs">Ouvrir</a></div>
                <div class="card"><h2>Sauvegardes planifiees</h2><p>Planification des sauvegardes de securite.</p><a class="btn" href="/ecritures/pro/sauvegardes">Ouvrir</a></div>
                <div class="card"><h2>Stripe</h2><p>Module abonnement pret a connecter.</p><a class="btn" href="/ecritures/pro/stripe">Ouvrir</a></div>
                <div class="card"><h2>Connecteurs bancaires</h2><p>Preparation DSP2 / banques.</p><a class="btn" href="/ecritures/pro/banques">Ouvrir</a></div>
                <div class="card"><h2>Rapprochement bancaire IA</h2><p>Matching operations / ecritures.</p><a class="btn" href="/ecritures/pro/rapprochement">Ouvrir</a></div>
                <div class="card"><h2>PDF bilan</h2><p>Generation synthese comptable imprimable.</p><a class="btn" href="/ecritures/pro/pdf-bilan">Ouvrir</a></div>
                <div class="card"><h2>Assistant IA</h2><p>Copilote comptable integre.</p><a class="btn" href="/ecritures/pro/assistant">Ouvrir</a></div>
                <div class="card"><h2>Deploiement cloud</h2><p>Checklist Render / VPS / Docker.</p><a class="btn" href="/ecritures/pro/deploiement">Ouvrir</a></div>
            </div>
        </div>
    </body>
    </html>
...
@ecritures_routes.route('/pro/utilisateurs')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_utilisateurs():

    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()
    c.execute("SELECT id, nom, email, role, actif, date_creation FROM utilisateurs_saas ORDER BY id")
    users = c.fetchall()
    conn.close()

    html = "<h1>Utilisateurs et roles</h1><table border='1' cellpadding='8'><tr><th>ID</th><th>Nom</th><th>Email</th><th>Role</th><th>Actif</th><th>Date</th></tr>"
    for u in users:
        html += f"<tr><td>{u[0]}</td><td>{u[1]}</td><td>{u[2]}</td><td>{u[3]}</td><td>{u[4]}</td><td>{u[5]}</td></tr>"
    html += "</table><p><a href='/ecritures/pro/dashboard'>Retour</a></p>"
    return html


@ecritures_routes.route('/pro/sauvegardes')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_sauvegardes():

    return """
    <h1>Sauvegardes planifiees</h1>
    <p>Statut : pret pour automatisation Windows Task Scheduler / cron.</p>
    <p>Commande recommandee :</p>
    <pre>C:\\Python314\\python.exe C:\\Users\\alain\\mon-projet-agent\\backup_auto.py</pre>
    <p><a href="/ecritures/production/sauvegarde">Lancer une sauvegarde maintenant</a></p>
    """


@ecritures_routes.route('/pro/stripe')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_stripe():
    ...
    return """
    <h1>Stripe SaaS</h1>
    <p>Module pret a connecter.</p>
    <ul>
        <li>Plan DEMO</li>
        <li>Plan CABINET</li>
        <li>Plan ENTREPRISE</li>
    </ul>
    <p>A brancher avec les cles STRIPE_SECRET_KEY et STRIPE_PRICE_ID.</p>
    """


@ecritures_routes.route('/pro/banques')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_banques():
    ...
    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()
    c.execute("SELECT id, banque, statut, date_creation FROM connecteurs_bancaires ORDER BY id DESC")
    rows = c.fetchall()
    conn.close()

    html = "<h1>Connecteurs bancaires</h1>"
    html += "<p>Preparation DSP2 / Bridge / Powens / Budget Insight.</p>"
    html += "<table border='1' cellpadding='8'><tr><th>ID</th><th>Banque</th><th>Statut</th><th>Date</th></tr>"
    for r in rows:
...
@ecritures_routes.route('/pro/rapprochement')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_rapprochement():
    ...
    return f"""
    <h1>Rapprochement bancaire IA</h1>
    <p>Operations a rapprocher : {nb}</p>
    <p>Moteur pret : matching montant, date, libelle, fournisseur.</p>
    <p><a href="/ecritures/rapprochement-bancaire">Ancien module rapprochement</a></p>
    """


@ecritures_routes.route('/pro/pdf-bilan')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_pdf_bilan():
    ...
    c.execute("""
        SELECT ROUND(SUM(COALESCE(debit,0)),2), ROUND(SUM(COALESCE(credit,0)),2)
        FROM ecritures
    """)
    debit, credit = c.fetchone()
    conn.close()
    ...
    return f"""
    <h1>Bilan comptable imprimable</h1>
    <p>Total debit : {debit}</p>
    <p>Total credit : {credit}</p>
    <p>Ecart : {round((debit or 0)-(credit or 0),2)}</p>
    <p>Generation PDF avancee a brancher ensuite.</p>
    """


@ecritures_routes.route('/pro/assistant')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_assistant():
    ...
    <p>Copilote integre pret a connecter a une API IA.</p>
    <ul>
        <li>Analyser une facture</li>
        <li>Expliquer une ecriture</li>
        <li>Verifier une balance</li>
        <li>Preparer une cloture</li>
        <li>Detecter anomalies</li>
    </ul>
    """


@ecritures_routes.route('/pro/deploiement')
@login_required
@permission_required("ACCESS_ECRITURES")
def pro_deploiement():

    return """
    <h1>Checklist deploiement cloud</h1>
    <ol>
        <li>Passer SQLite vers PostgreSQL pour SaaS multi-clients.</li>
        <li>Ajouter variables environnement SECRET_KEY, DATABASE_URL, STRIPE_SECRET_KEY.</li>
        <li>Utiliser Gunicorn ou Waitress en production.</li>
        <li>Activer HTTPS.</li>
        <li>Planifier sauvegardes quotidiennes.</li>
        <li>Ajouter monitoring erreurs.</li>
        <li>Tester FEC, balance, OCR, multi-dossiers.</li>
    </ol>
    """



@ecritures_routes.route('/operationnel')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_dashboard():
    ...
    <head>
        <meta charset="utf-8">
        <title>Operationnel comptable</title>
        <style>
            body{font-family:Arial;background:#f4f6f9;padding:40px;color:#1f2d3d;}
            h1{color:#173b66;}
            .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:18px;max-width:1100px;}
            .card{background:white;border-radius:10px;padding:22px;box-shadow:0 2px 8px rgba(0,0,0,.08);border-left:5px solid #173b66;}
            .btn{display:inline-block;background:#173b66;color:white;padding:10px 14px;border-radius:6px;text-decoration:none;font-weight:bold;}
        </style>
    </head>
    <body>
        <h1>Modules operationnels</h1>
        <div class="grid">
            <div class="card">
                <h2>Plan comptable par societe</h2>
                <p>Controle du plan comptable isole par dossier.</p>
                <a class="btn" href="/ecritures/operationnel/plan-comptable">Ouvrir</a>
            </div>
            <div class="card">
                <h2>Immobilisations</h2>
                <p>Fiches immobilisations et tableaux d'amortissement.</p>
                <a class="btn" href="/ecritures/operationnel/immobilisations">Ouvrir</a>
            </div>
            <div class="card">
                <h2>Prets bancaires</h2>
                <p>Tableaux d'amortissement des emprunts.</p>
                <a class="btn" href="/ecritures/operationnel/prets">Ouvrir</a>
            </div>
        </div>
    </body>
    </html>
...
@ecritures_routes.route('/operationnel/plan-comptable')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_plan_comptable():
    ...
    html = f"<h1>Plan comptable dossier {dossier_id}</h1>"
    html += "<p><a href='/ecritures/operationnel/import-plan-modele'>Importer plan modele</a></p>"
    html += "<table border='1' cellpadding='8'><tr><th>Numero</th><th>Libelle</th><th>Type</th></tr>"

    for cpte in comptes:
        html += f"<tr><td>{cpte[0]}</td><td>{cpte[1]}</td><td>{cpte[2]}</td></tr>"

    html += "</table><p><a href='/ecritures/operationnel'>Retour</a></p>"

    return html


@ecritures_routes.route('/operationnel/import-plan-modele')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_import_plan_modele():
    ...
    plan = [
        ...
        ('201000', 'Frais etablissements', 'Immobilisation'),
        ('205000', 'Concessions brevets logiciels', 'Immobilisation'),
        ('213000', 'Constructions', 'Immobilisation'),
        ('215000', 'Installations techniques', 'Immobilisation'),
        ('218100', 'Installations generales', 'Immobilisation'),
        ('218200', 'Materiel de transport', 'Immobilisation'),
        ('218300', 'Materiel bureau informatique', 'Immobilisation'),
        ('280500', 'Amortissements logiciels', 'Amortissement'),
        ('281300', 'Amortissements constructions', 'Amortissement'),
        ('281500', 'Amortissements installations techniques', 'Amortissement'),
        ('281830', 'Amortissements materiel informatique', 'Amortissement'),
        ('401000', 'Fournisseurs', 'Passif'),
        ('411000', 'Clients', 'Actif'),
        ('445660', 'TVA deductible', 'Actif'),
        ('445710', 'TVA collectee', 'Passif'),
        ('512000', 'Banque', 'Tresorerie'),
        ('606300', 'Fournitures entretien petit equipement', 'Charge'),
        ('615000', 'Entretien et reparations', 'Charge'),
        ('616000', 'Assurances', 'Charge'),
        ('626000', 'Frais postaux telecom', 'Charge'),
        ('627000', 'Services bancaires', 'Charge'),
        ('661000', 'Charges interets', 'Charge'),
        ('681120', 'Dotations amortissements immobilisations', 'Charge'),
    ...
    for numero, libelle, type_compte in plan:
        c.execute("""
            SELECT id FROM plan_comptable
            WHERE numero=? AND COALESCE(dossier_id,1)=?
        """, (numero, dossier_id))
        ...
        if not c.fetchone():
            c.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (numero, libelle, type_compte, dossier_id))

    conn.commit()
    conn.close()

    flash("Plan comptable modele importe pour le dossier courant.", "success")
    return redirect("/ecritures/operationnel/plan-comptable")


@ecritures_routes.route('/operationnel/immobilisations')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_immobilisations():
    ...
    c.execute("""
        SELECT id, libelle, date_acquisition, valeur_origine, compte_immobilisation,
               duree_mois, mode, statut
    ...
    html = f"<h1>Immobilisations dossier {dossier_id}</h1>"
    html += "<p><a href='/ecritures/operationnel/immobilisation-demo'>Creer immobilisation demo</a></p>"
    html += "<table border='1' cellpadding='8'><tr><th>ID</th><th>Libelle</th><th>Date</th><th>Valeur</th><th>Compte</th><th>Duree mois</th><th>Mode</th><th>Statut</th><th>Tableau</th></tr>"

    for i in immos:
        html += f"<tr><td>{i[0]}</td><td>{i[1]}</td><td>{i[2]}</td><td>{i[3]}</td><td>{i[4]}</td><td>{i[5]}</td><td>{i[6]}</td><td>{i[7]}</td><td><a href='/ecritures/operationnel/immobilisation/{i[0]}/tableau'>Voir</a></td></tr>"

    html += "</table><p><a href='/ecritures/operationnel'>Retour</a></p>"
    return html


@ecritures_routes.route('/operationnel/immobilisation-demo')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_immobilisation_demo():
    ...
    c.execute("""
        ...
        (dossier_id, libelle, date_acquisition, valeur_origine,
         compte_immobilisation, compte_amortissement, compte_dotation, duree_mois, mode)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        dossier_id,
        "Ordinateur portable demo",
        "2026-01-01",
        1200.00,
        "218300",
        "281830",
    ...
    immo_id = c.lastrowid
    dotation = round(1200.00 / 36, 2)
    cumul = 0

    for mois in range(1, 37):
        cumul = round(cumul + dotation, 2)
        vnc = round(max(1200.00 - cumul, 0), 2)
        periode = f"2026-M{mois:02d}"

        c.execute("""
            INSERT INTO amortissements_immobilisations
            (immobilisation_id, dossier_id, periode, dotation, cumul_amortissement, valeur_nette)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (immo_id, dossier_id, periode, dotation, cumul, vnc))

    conn.commit()
    conn.close()

    flash("Immobilisation demo creee avec tableau d'amortissement.", "success")
    return redirect("/ecritures/operationnel/immobilisations")


@ecritures_routes.route('/operationnel/immobilisation/<int:immo_id>/tableau')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_immobilisation_tableau(immo_id):

    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()

    c.execute("""
        SELECT periode, dotation, cumul_amortissement, valeur_nette, statut
        FROM amortissements_immobilisations
    ...
    html = f"<h1>Tableau amortissement immobilisation {immo_id}</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>Periode</th><th>Dotation</th><th>Cumul</th><th>VNC</th><th>Statut</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td><td>{l[3]}</td><td>{l[4]}</td></tr>"

    html += "</table><p><a href='/ecritures/operationnel/immobilisations'>Retour</a></p>"
    return html


@ecritures_routes.route('/operationnel/prets')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_prets():
    ...
    c.execute("""
        SELECT id, libelle, banque, capital_initial, taux_annuel, duree_mois, date_debut, statut
    ...
    html = f"<h1>Prets bancaires dossier {dossier_id}</h1>"
    html += "<p><a href='/ecritures/operationnel/pret-demo'>Creer pret demo</a></p>"
    html += "<table border='1' cellpadding='8'><tr><th>ID</th><th>Libelle</th><th>Banque</th><th>Capital</th><th>Taux</th><th>Duree</th><th>Date debut</th><th>Statut</th><th>Tableau</th></tr>"

    for p in prets:
        html += f"<tr><td>{p[0]}</td><td>{p[1]}</td><td>{p[2]}</td><td>{p[3]}</td><td>{p[4]}</td><td>{p[5]}</td><td>{p[6]}</td><td>{p[7]}</td><td><a href='/ecritures/operationnel/pret/{p[0]}/tableau'>Voir</a></td></tr>"

    html += "</table><p><a href='/ecritures/operationnel'>Retour</a></p>"
    return html


@ecritures_routes.route('/operationnel/pret-demo')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_pret_demo():
    ...
    mensualite = round(capital * taux_mensuel / (1 - (1 + taux_mensuel) ** (-duree_mois)), 2)

    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()

    c.execute("""
        INSERT INTO prets_bancaires
        (dossier_id, libelle, banque, capital_initial, taux_annuel, duree_mois, date_debut, assurance_mensuelle)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        dossier_id,
        "Pret bancaire demo",
    ...
    for n in range(1, duree_mois + 1):
        interets = round(restant * taux_mensuel, 2)
        capital_rembourse = round(mensualite - interets, 2)
        restant = round(max(restant - capital_rembourse, 0), 2)
        date_ech = f"2026-M{n:02d}"

        c.execute("""
            INSERT INTO echeances_prets
            (pret_id, dossier_id, numero_echeance, date_echeance, mensualite,
             interets, capital_rembourse, assurance, capital_restant)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            pret_id,
            dossier_id,
    ...
    flash("Pret demo cree avec tableau d'amortissement.", "success")
    return redirect("/ecritures/operationnel/prets")


@ecritures_routes.route('/operationnel/pret/<int:pret_id>/tableau')
@login_required
@permission_required("ACCESS_ECRITURES")
def operationnel_pret_tableau(pret_id):

    conn = sqlite3.connect("db.sqlite")
    c = conn.cursor()

    c.execute("""
        SELECT numero_echeance, date_echeance, mensualite, interets,
               capital_rembourse, assurance, capital_restant, statut
    ...
    html = f"<h1>Tableau amortissement pret {pret_id}</h1>"
    html += "<table border='1' cellpadding='8'><tr><th>No</th><th>Date</th><th>Mensualite</th><th>Interets</th><th>Capital</th><th>Assurance</th><th>Restant</th><th>Statut</th></tr>"

    for l in lignes:
        html += f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td><td>{l[3]}</td><td>{l[4]}</td><td>{l[5]}</td><td>{l[6]}</td><td>{l[7]}</td></tr>"
